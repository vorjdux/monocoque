...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let port = 15570 + (std::time::SystemTime::now()
    ...
    let dealer_task = compio::runtime::spawn(async move {
        dealer_client(&addr_clone).await
    });
...
async fn router_server(addr: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, frame) in request.iter().enumerate() {
        println!("[ROUTER]   Frame {}: {:?}", i, String::from_utf8_lossy(frame));
    }
    ...
    if request.len() >= 3 {
        ...
        println!("[ROUTER] Message: {:?}", String::from_utf8_lossy(&payload[0]));
...
async fn dealer_client(addr: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, frame) in reply.iter().enumerate() {
        println!("[DEALER]   Frame {}: {:?}", i, String::from_utf8_lossy(frame));
    }
...
fn main() {
    ...
    eprintln!("Run with: cargo run --example router_dealer_basic --features runtime");
}
