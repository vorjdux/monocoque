//! One program, either runtime backend, no code changes.
//!
...
//! both backends; you pick the backend with a Cargo feature:
//!
//! ```bash
...
//! ```
//!
//! `LocalRuntime` wraps a single-threaded runtime (a compio runtime, or a
//! current-thread tokio runtime inside a `LocalSet`), so `rt::spawn` and the
//! socket calls behave the same either way.
